...
import { DocumentData, DocumentSnapshot, doc, getDoc, onSnapshot, setDoc } from 'firebase/firestore'
...
export const useFirestoreDoc = <T = any>(docPath: string) => {
  ...
  useEffect(() => {
    ...
    async function init(_snapshot: DocumentSnapshot<DocumentData> | null = null) {
      try {
