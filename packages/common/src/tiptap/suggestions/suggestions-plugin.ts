...
export type SuggestionsViewProps = SuggestionsPluginProps & {
  view: EditorView
}
...
export class SuggestionsView {
  ...
  public shouldShow: Exclude<SuggestionsPluginProps['shouldShow'], null> = ({ view, state, from, to }) => {
    ...
    if (!hasEditorFocus || empty || isEmptyTextBlock || !hasMark || !this.editor.isEditable) {
      return false
    }
  ...
  constructor({ editor, element, view, tippyOptions = {}, updateDelay = 250, shouldShow }: SuggestionsViewProps) {
    this.editor = editor
...
export const SuggestionsPlugin = (options: SuggestionsPluginProps) => {
  return new Plugin({
    key: typeof options.pluginKey === 'string' ? new PluginKey(options.pluginKey) : options.pluginKey,
    view: view => new SuggestionsView({ view, ...options }),
  })
}
