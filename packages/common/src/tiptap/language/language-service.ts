...
export const fetchCategories = async (): Promise<BiasCategory[]> => {
  ...
  const result = await fetch(`${BIAS_URL}/api/v1/categories`).then((r) =>
    r.json()
  );
...
export const fetchCompletions = async (
  input: string
): Promise<BiasCompletions> => {
  console.log("Completions/FETCHING: ");

  const response = await fetch(
    `${BIAS_URL}/api/v1/completions`,
    apiBody(input)
  ).then((r) => r.json());
...
export const fetchClassifications = async (
  input: string
): Promise<BiasClassResult[]> => {
  console.log("Completions/FETCHING: ", input);
  const result = await fetch(
    `${BIAS_URL}/api/v1/classifications`,
    apiBody(input)
  ).then((r) => r.json());
  console.log("Completions/RESPONSE: ", result);
...
export const fetchBiases = async (input: string): Promise<Bias> => {
  ...
  const result = await fetch(`${BIAS_URL}/check`, apiBody(input)).then((r) =>
    r.json()
  );
